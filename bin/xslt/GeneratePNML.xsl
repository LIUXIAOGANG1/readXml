...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="net/labels">
		<xsl:element name="labels">
			<xsl:attribute name="x"><xsl:value-of select="@positionX"/></xsl:attribute>
			<xsl:attribute name="y"><xsl:value-of select="@positionY"/></xsl:attribute>
	...
	<xsl:template match="net/definitions">
		<xsl:element name="definition">
			<xsl:attribute name="defType"><xsl:value-of select="@defType"/></xsl:attribute>
	...
	<xsl:template match="net/transition">
		<xsl:element name="transition">
			...
			<xsl:call-template name="endtime"/>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="arcpath">
		<xsl:element name="arcpath">
			...
			<xsl:attribute name="curvePoint"><xsl:value-of select="@arcPointType"/></xsl:attribute>
		</xsl:element>
	</xsl:template>
			...
			<xsl:element name="graphics">
				<xsl:if test="(string-length(@nameOffsetX) > 0)and (string-length(@nameOffsetY) > 0)">
					<xsl:element name="offset">
						<xsl:attribute name="x"><xsl:value-of select="@nameOffsetX"/></xsl:attribute>
						<xsl:attribute name="y"><xsl:value-of select="@nameOffsetY"/></xsl:attribute>
					</xsl:element>
				</xsl:if>
			</xsl:element>
		</xsl:element>
		...
		<xsl:element name="initialMarking">
			...
			<xsl:element name="graphics">
				<xsl:if test="(string-length(@markingOffsetX) > 0) and (string-length(@markingOffsetY) > 0)">
					<xsl:element name="offset">
						<xsl:attribute name="x"><xsl:value-of select="@markingOffsetX"/></xsl:attribute>
						<xsl:attribute name="y"><xsl:value-of select="@markingOffsetY"/></xsl:attribute>
					</xsl:element>
				</xsl:if>
			</xsl:element>
		</xsl:element>
		...
		<xsl:element name="inscription">
			...
			<xsl:element name="graphics">
				<xsl:if test="(string-length(@inscriptionOffsetX) > 0)  and (string-length(@inscriptionOffsetY) > 0)">
					<xsl:element name="offset">
						<xsl:attribute name="x"><xsl:value-of select="@inscriptionOffsetX"/></xsl:attribute>
						<xsl:attribute name="y"><xsl:value-of select="@inscriptionOffsetY"/></xsl:attribute>
					</xsl:element>
				</xsl:if>
			</xsl:element>
		</xsl:element>
	...
	<xsl:template name="transitionRateParameter">
		<xsl:if test="string-length(@parameter)>0">
			<xsl:element name="toolspecific">
				<xsl:attribute name="tool"><xsl:text>PIPE</xsl:text></xsl:attribute>
				<xsl:attribute name="version"><xsl:text>2.5</xsl:text></xsl:attribute>
				<xsl:attribute name="rateDefinition"><xsl:value-of select="@parameter"/></xsl:attribute>
				<!--            <xsl:element name="name">
               <xsl:element name="value">
	...
	<xsl:template name="placeMarkingParameter">
		<xsl:if test="string-length(@parameter)>0">
			<xsl:element name="toolspecific">
				<xsl:attribute name="tool"><xsl:text>PIPE</xsl:text></xsl:attribute>
				<xsl:attribute name="version"><xsl:text>2.5</xsl:text></xsl:attribute>
				<xsl:attribute name="markingDefinition"><xsl:value-of select="@parameter"/></xsl:attribute>
				<!--            <xsl:element name="name">
               <xsl:element name="value">
                  <xsl:value-of select="@parameter"/>
               </xsl:element>         
