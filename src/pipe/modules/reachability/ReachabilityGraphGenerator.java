...
import pipe.dataLayer.calculations.StateSpaceGenerator;// Important
...
import pipe.io.RGFileHeader; //im
import pipe.io.StateRecord; //im
import pipe.io.TransitionRecord;//im
...
public class ReachabilityGraphGenerator 
        implements Module {
	//½ðÁú
	public static java.util.ArrayList edges1=new ArrayList();
	public static java.util.ArrayList nodes1=new ArrayList();
	public static TransitionRecord newtrans = new TransitionRecord();
	//public static RandomAccessFile reachabilityFile1;
	//½ðÁú ½áÊø
   ...
   private static DefaultGraph createGraph(File rgFile, DataLayer dataLayer, 
      ...
      int i = nodes.size();
      for (int k=0;k<i;k++){
    	 nodes1.add(k, nodes.get(k));
    	 
      }
      int j = edges.size();
      for (int k=0;k<j;k++){
    	 edges1.add(k, edges.get(k));    	 
      }
