 ...
 //du: 用于MRG analysis菜单；
   ...
   private void buildMenus() {
      ...
      addMenuItem(MRGMenu, limitFrame = 
          new MRGAction("添加时间限制", Pipe.LIMITFRAME, "time limit","ctrl shift .",true));
      drawMenu.addSeparator();
      addMenuItem(MRGMenu, Analysis1Action = 
          new MRGAction("分析结果", Pipe.ANALYZE1, "time analysis","ctrl shift ,",true));
      drawMenu.addSeparator();
    // du: MRG Analysis菜单结束。 
   ...
   private void buildToolbar() {
      ...
      //金龙 添加一个“添加interval的transition的按钮”
...
// du: 为了analysis MRG菜单；
   class MRGAction extends GuiAction {
	      ...
	      private void copyTab(int from , int to, int x, int y){
              ...
              if(to==-1){
            	  createNewTab(null,false);
            	  composetab =appTab.getSelectedIndex(); //没法引用传递
	    	  }else{
...
//			 {"t9", "p16","out","gg"},
		...
		private void ShowFrame4() {
			...
			limitLabel = new javax.swing.JLabel("请输入限制时间：");
		...
		// numAry是arraynode和arrayedge的size
		public int numAry = 0;
		public int numNd = 0;
		// 用于显示的字符串
		...
		public void multPath() {
				...
				while (((DefaultNode) (arraynode.get(n)).get(arraynode.get(n)
					...
					if (edgesTo.size() > 1) {
						...
						for (int k = 1; k < i; k++) {
							...
							// 如果有分支，在arraynode中新建一个List，并使新建的list等于原list
							// 然后，将分支后的节点分别加入到这几个新list中
			...
			for (int i = 0; i < arrayedge.size(); i++) {
				...
				str += "\r\n"+"其这条服务路径处理时间为：" + "["+startsum+","+endsum+"]";
	    		  if (limittime > endsum){
	    			  str += "\r\n服务将在限定时间之内完成,可选方案。";
	    		  }
	    		  else if(limittime < startsum){
	    			  str += "\r\n服务将不能再限定时间之内完成，需改进或者选择其他服务路径";
	    		  }
	    		  else {
	    			  str += "\r\n服务可能在限定时间内完成，也可能在限定时间之外完成，可以选择，但是有改进的必要";
				}
