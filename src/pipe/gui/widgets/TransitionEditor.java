...
// @金龙 这里是TransitionEditor,有关属性面板的内容在这里。这里可以对transition属性的显示做一些修改。
   ...
   public TransitionEditor(JRootPane _rootPane, Transition _transition, 
      ...
      interval = transition.isTimeInterval();
      ...
      //因为原来这里是有两个状态，因此这样判断，现在是三个状态，需要改变这个判断语句。//已修改
      if (timed){
         timedTransition();
      } else if(interval){timeintervalTransition();}
      	else {
         immediateTransition();
...
//	      }      
   ...
   private void immediateTransition(){
      ...
      priorityTextField.setEnabled(true);
   ...
   // 下面是属性面板的布局 ,gridBagConstraints中声明的东西才能在transitionEditor中显示出来，
   //若存在没有声明的组件，则transitionEditor整个面板也不能显示出来。
   private void initComponents() {
      ...
      attributesCheckBox.setSelected(true);
      ...
      //金龙 修改 还要添加文本框的监听器，
      //并且使它与getstarttime()和getendtime()相关联。
      ...
      gridBagConstraints.insets = new java.awt.Insets(3, 3, 3, 3);
      transitionEditorPanel.add(starttimeLabel, gridBagConstraints);
      
      starttimeTextField.setText(""+transition.getStarttime());
      ...
      gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
      gridBagConstraints.insets = new java.awt.Insets(3, 3, 3, 3);
      transitionEditorPanel.add(starttimeTextField, gridBagConstraints);
      
      endtimeTextField.setText(""+transition.getEndtime());
      ...
      gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
      gridBagConstraints.insets = new java.awt.Insets(3, 3, 3, 3);
      transitionEditorPanel.add(endtimeTextField, gridBagConstraints);
      ...
      gridBagConstraints.anchor = java.awt.GridBagConstraints.WEST;
      gridBagConstraints.insets = new java.awt.Insets(3, 3, 3, 3);
      transitionEditorPanel.add(endtimeLabel, gridBagConstraints);
    //金龙修改结束，此处在transitionEditor中加了starttime，和endtime以及对应的文本框  
      ...
      timingButtonGroup.add(timeintervalRadioButton);
      timeintervalRadioButton.setText("interval");
      timeintervalRadioButton.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
      timeintervalRadioButton.setMargin(new java.awt.Insets(0, 0, 0, 0));
      timeintervalRadioButton.setMaximumSize(new java.awt.Dimension(90, 15));
      timeintervalRadioButton.setMinimumSize(new java.awt.Dimension(90, 15));
      timeintervalRadioButton.setPreferredSize(new java.awt.Dimension(90, 15));
      timeintervalRadioButton.addActionListener(new java.awt.event.ActionListener() {
         public void actionPerformed(java.awt.event.ActionEvent evt) {
        	 timeintervalRadioButtonActionPerformed(evt);
         }
      });
      timingPanel.add(timeintervalRadioButton);
   ...
   //金龙 修改
   private void  timeintervalRadioButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_immediateRadioButtonActionPerformed
	      if ( timeintervalRadioButton.isSelected()){
	         timeintervalTransition();
	      } else {
	         timedTransition();
	      }
	   }
   ...
   private void okButtonHandler(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_okButtonHandler
      ...
      if(timedRadioButton.isSelected())
      {
    	  ...
    	  transition.setTimeInterval(false);
      }
      else if (timeintervalRadioButton.isSelected())
      {transition.setTimeInterval(true);
       transition.setTimed(false);}
      else if (immediateRadioButton.isSelected())
      {transition.setTimed(false);
      transition.setTimeInterval(false);}
      //金龙修改 结束，根据transition的类型确定显示内容。
      ...
      if(timeintervalRadioButton.isSelected())
      {    
    	  int newstarttime = Integer.parseInt(starttimeTextField.getText());
    	  int newendtime = Integer.parseInt(endtimeTextField.getText());
    	  if(newstarttime!=transition.getStarttime()){
    		  view.getUndoManager().addEdit(transition.setStarttime(newstarttime));
    	  }
    	  if(newendtime!= transition.getEndtime()){
    		  view.getUndoManager().addEdit(transition.setEndtime(newendtime));
    	  }
    	  /*
    	  if (Double.parseDouble(starttimeTextField.getText())!=transition.getStarttime())
    	  {transition.setStarttime(Integer.parseInt(starttimeTextField.getText()));};
    	  if (Double.parseDouble(endtimeTextField.getText())!=transition.getStarttime())
    	  {transition.setEndtime(Integer.parseInt(endtimeTextField.getText()));};
    	  */
      };
