...
import pipe.gui.undo.TransitionEndtimeEdit;
import pipe.gui.undo.TransitionServerSemanticEdit;
import pipe.gui.undo.TransitionStarttimeEdit;
...
public class Transition
   ...
   //金龙修改 ，添加两个整形变量代表transition的开始和结束时间
   private int starttime;
   private int endtime;
   //金龙修改  ，添加一个string变量，用来表达【starttime，endtime】
   private String timeinterval ;   
   //timeinterval =("["+ transition.getstarttime()+","+ transition.getendtime()+"]");
   private boolean isTimeInterval = false;
   //金龙修改，isTimeInterval 用来判断是否以区间形式显示transition的时间属性。
    ...
    */
   // 金龙 此处transition函数中的变量都是他的属性，其时间属性的需要在这里修改函数,我在这里添加了Transition的三个变量
   //starttime,endtime,isTimeinterval.
   public Transition(double positionXInput, double positionYInput, 
                     ...
                     int angleInput,
                     int priority,
                     int strtime,int entime,boolean timeInterval){
   ...
   public Transition paste2(double x, double y, boolean fromAnotherView){
	      Transition copy = new Transition (
	              Grid.getModifiedX(x + this.getX() + Pipe.PLACE_TRANSITION_HEIGHT/2),
	              Grid.getModifiedY(y + this.getY() + Pipe.PLACE_TRANSITION_HEIGHT/2));
	      copy.pnName.setName(this.pnName.getName() /* + "(" + this.getCopyNumber() +")"*/);
  ...
  // ------------------------------------------------------***********
   public UndoableEdit setStarttime(int newStarttime){
	   int oldstarttime = starttime;
	   starttime =new Integer(newStarttime);
	   pnName.setText(getText());
	   repaint();
	   return new TransitionStarttimeEdit(this,oldstarttime,starttime);
	   
   }
   public UndoableEdit setEndtime(int newEndtime){
	   int oldendtime = endtime;
	   endtime = new Integer(newEndtime);
	   pnName.setText(getText());
	   repaint();
	   return new TransitionEndtimeEdit(this,oldendtime,endtime);
   }
   ...
   private String getText(){
      if (attributesVisible == true){
         ...
         else if (isTimeInterval() && !isTimed()){
        	return "\n["+starttime+","+endtime+"]";} 
         else if (!isTimed() && !isTimeInterval()){
            if (rateParameter!= null){
...
public void setTimeInterval(boolean isTimeInterval) {
